...
import { summarizeWithCodex } from "./codex-summarizer.js";
...
function spokenName(slot){const name=(slot.session&&slot.session.project)||'Codex';const occurrences=(latestState?latestState.slots:[]).filter(function(other){return other.session&&(((other.session.project)||'Codex').toLowerCase()===name.toLowerCase())}).length;return occurrences>1?'Session '+slot.slot:name}
function questionKey(session){return session.state+'|'+(session.waiting_kind||'')+'|'+(session.question||'')}
function questionAnnouncement(slot){const session=slot.session;const question=session.question||'Approval requested';let text=spokenName(slot)+' is asking: '+question+(question.endsWith('.')?'':'.');if(slot.risk==='high')text+=' High risk: '+pendingDescription(session)+'. Say '+CONVERSATION_POLICY.confirmPhrase+' to allow, or no to deny.';else text+=' Say yes to approve, or no to deny.';return text}
function buildDigest(){const parts=[];let waitingCount=0;(latestState?latestState.slots:[]).forEach(function(slot){const session=slot.session;if(!session)return;let status='ready';if(session.state==='waiting'){status='waiting for approval';waitingCount++}else if(session.state==='busy')status='working';parts.push(spokenName(slot)+' is '+status)});if(!parts.length)return 'No sessions are running.';let summary=parts.length===1?'One session':parts.length+' sessions';if(waitingCount)summary+=waitingCount===1?', one needs you':', '+waitingCount+' need you';return summary+'. '+parts.join('. ')+'.'}
...
export async function startServer(
  store: StateStore,
  port = 4917,
  summarizer: (text: string) => Promise<string | null> = summarizeWithCodex,
): Promise<{ close: () => Promise<void>; port: number; token: string }> {
  ...
  const server = createServer(async (request, response) => {
    ...
    if (request.method === "POST" && url.pathname === "/api/summarize") {
      try {
        const payload = await body(request) as { slot?: unknown };
        const slot = typeof payload?.slot === "number" ? payload.slot : 0;
        const session = (await store.getGrid()).slots.find((entry) => entry.slot === slot)?.session;
        if (!session?.last_message) return json(response, 404, { error: "Nothing to summarize for that session." });
        const summary = await summarizer(session.last_message);
        if (!summary) return json(response, 502, { error: "The summarizer is unavailable." });
