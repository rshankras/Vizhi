...
async function handleConverseIntent(intent){const selected=selectedSessionData();if(intent.intent==='approve'||intent.intent==='confirm_approve'){if(!selected||selected.session.state!=='waiting'){converseSpeak('That request is no longer pending.',false);return}if(intent.intent==='approve'&&selected.risk==='high'){converseSpeak('High risk: '+pendingDescription(selected.session)+'. Say '+CONVERSATION_POLICY.confirmPhrase+' to allow, or no to deny.',true);return}await queueAction({type:'approve'},true);converseSpeak('Approved.',false);return}if(intent.intent==='deny'){if(!selected||selected.session.state!=='waiting'){converseSpeak('That request is no longer pending.',false);return}await queueAction({type:'deny'},true);converseSpeak('Denied.',false);return}if(intent.intent==='status'){converseSpeak(buildDigest(),false);return}if(intent.intent==='focus_session'){const target=latestState&&latestState.slots.find(function(slot){return slot.slot===intent.slot&&slot.session});if(!target){converseSpeak('Session '+intent.slot+' is empty.',false);return}selectedSlot=intent.slot;renderState();await queueAction({type:'focus'},true);if(target.session.state==='waiting'){announcedQuestions[target.session.session_id]=questionKey(target.session);converseSpeak('Switched to '+(target.session.project||'Codex')+'. '+questionAnnouncement(target),true)}else converseSpeak('Switched to '+(target.session.project||'Codex')+'.',false);return}if(intent.intent==='screenshot'){await queueAction({type:'screenshot'},true);converseSpeak('Screenshot started. Select an area, then speak your context.',false);return}if(intent.intent==='mute'){setConverseMuted(true);return}if(intent.intent==='end_conversation'){stopConversation('Goodbye.');return}await queueAction({type:'voice',text:intent.text},true);converseSpeak('Sent to '+((selected&&selected.session.project)||'the session')+'.',false)}
...
micState.addEventListener('click',function(){if(conversing)setConverseMuted(!converseMuted)});
