...
function renderState(){if(!latestState)return;meta.textContent=latestState.overflow?latestState.overflow+' unslotted session(s)':'Local IPC · live';renderSessions();renderSelectedSession();updateAvailability()}
function renderTemplates(templates){const groups={prompts:[],git:[]};templates.forEach(function(template){const target=template.group==='Vizhi Git'?groups.git:groups.prompts;target.push('<button class="control" data-action="prompt_template" data-template-id="'+escapeMarkup(template.id)+'" data-needs-session>'+escapeMarkup(template.label)+'</button>')});promptControls.innerHTML=groups.prompts.join('');gitControls.innerHTML=groups.git.join('');updateAvailability()}
function renderFavorite(template){favoriteControl.innerHTML='<button class="control primary" data-action="favorite" data-needs-session>★ '+escapeMarkup(template.label)+'</button>';updateAvailability()}
...
document.querySelector('#voice').addEventListener('click',function(){const Recognition=window.SpeechRecognition||window.webkitSpeechRecognition;if(!Recognition){setNotice('Browser voice is unavailable here. Type text and press Send.',true);return}const voiceButton=this;const recognition=new Recognition();recognition.lang=navigator.language||'en-US';recognition.interimResults=false;recognition.maxAlternatives=1;voiceButton.disabled=true;voiceButton.textContent='Listening…';setNotice('Listening…');recognition.onresult=function(event){const text=(event.results[0][0].transcript||'').trim();if(!text){setNotice('No speech was captured. Try Voice again or type text.',true);return}composerText.value=text;queueAction({type:'voice',text:text},true).then(function(){composerText.value=''}).catch(function(error){setNotice('Voice captured. '+error.message+' You can press Send to retry.',true)})};recognition.onerror=function(){setNotice('Voice failed. Type text and press Send.',true)};recognition.onend=function(){voiceButton.textContent='Voice';updateAvailability()};recognition.start()});
...
function renderedPage(token: string): string {
  return PAGE.replace("__VIZHI_TOKEN__", token);
}
