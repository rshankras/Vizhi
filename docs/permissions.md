...
- **Microphone — needed for Voice.** Browser Voice asks your browser for microphone access and may use an external browser speech-recognition service. After physical Voice finishes its one-time setup, `Vizhi Voice Helper` asks for access and transcribes locally with Whisper. Voice conversation reuses the same grants and adds no permissions: keypad speech output uses macOS's built-in voice, and browser announcements use the browser's own speech synthesis.
