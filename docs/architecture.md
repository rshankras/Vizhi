...
Voice conversation keeps the same boundaries. Keypad conversations listen through the local Whisper helper and speak through macOS's built-in `say` voice; browser conversations use the browser's speech services behind the same disclosure. Vizhi speaks text the Codex hooks provide — approval questions, pending tool and command names, session states, and project names — plus one deliberate addition: when a turn completes, the hook records the final assistant message (capped, from the hook payload or the local transcript tail) so completion announcements can speak a short, code-stripped summary and answer "read more". That text stays in the local session file and is only ever spoken on this Mac; Vizhi still never reads or transmits full conversation history.

The spoken `summarize` command is the one deliberate exception to local-only processing, and it only runs when explicitly asked. It sends that single stored answer back to Codex in a headless, ephemeral `codex exec` run — text that originated from the same model provider, so no new data is exposed — using the account's default model with your normal Codex quota. The run loads no user config (so no hooks or MCP servers), persists no session files, and appears in no resume list; automatic announcements never use it. Spoken approvals, focus changes, screenshots, and prompts are written as the same normalized action records as key presses, so the waiting-state gate and TTY verification always apply, and a high-risk approval additionally requires the spoken `confirm approve` phrase. Conversation mode is a per-surface interaction state; it adds no new IPC files and no shared state.
