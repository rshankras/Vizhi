...
`Converse` starts a spoken conversation and shows a microphone pill in the header. The dashboard announces approval questions (with a spoken readback and a required `confirm approve` phrase for high-risk commands), finished sessions with a short code-stripped summary of the answer (`read more` speaks a longer passage; `summarize that` asks Codex itself for a true two-sentence summary through an ephemeral `codex exec` run on your account), ended sessions, and answers `status`, `switch to session two`, `take a screenshot`, `mute`, and `end conversation`; anything else is sent to the selected session as a prompt. Spoken approvals create the same verified action records as the `Yes` and `No` buttons. Click the microphone pill whenever you want to talk — it opens listening while monitoring, interrupts an announcement mid-sentence, or closes an open microphone; say `mute` to keep the microphone closed after announcements. The same speech-recognition disclosure applies as for `Browser Voice`; announcements use the browser's speech synthesis. In browsers without speech recognition, `Converse` explains that and stays off — use the keypad's fully local conversation instead. An idle conversation ends itself after 10 minutes, and the conversation ends automatically when the last session does.
